...
/*   Updated: 2023/07/18 21:04:17 by briferre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	*vector_rotate(double *v, double angle)
{
	...
	r[0] = cos(degrees_to_radians(angle)) * v[0];
	r[1] += sin(degrees_to_radians(angle)) * v[1];
	r[1] = -sin(degrees_to_radians(angle)) * v[0];
	r[1] += cos(degrees_to_radians(angle)) * v[1];
	return (r);
}
